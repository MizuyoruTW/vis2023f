...
<h3>從上圖中，我們可以得出：
  <ul>
    <li>問卷超過1/3的受訪者的工作地點是工作室</li>
    <li>「減少包裝」、「節約能源」是大多數人採取減碳行動的前三名</li>
  </ul>
</h3>`
  )
}
...
function _color(d3) {
  return (
    d3
      .scaleOrdinal()
      .domain(["工作室", "替代空間", "美術館", "減少包裝材及文宣印製", "使用無毒媒材、再生材料、廢物利用素材等", "工作場所、活動展場的節約能源", "減少商務差旅", "選擇符合永續精神的活動場地", "非營利組織/藝術團體"])
      //.range(d3.schemePaired)
      .range(["#D0104C", "#F17C67", "#FFB11B", "#BEC23F", "#3A8FB7", "#8F77B5", "#C1328E", "#787878", "#24936E"])
      .unknown("#BDC0BA")
  )
}
...
function _breadcrumbPoints(breadcrumbWidth, breadcrumbHeight) {
  return (
    function breadcrumbPoints(d, i) {
      ...
      const points = [];
      if (i > 0) {
        points.push(`${tipWidth},0`);
      } else {
        points.push("0,0");
      }
      points.push(`${breadcrumbWidth + tipWidth},0`);
