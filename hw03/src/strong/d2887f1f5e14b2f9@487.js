...
function _taiwanMap(d3, topojson, tw, DOM, bgColor, strokeColor, strokeOpacity, minidata) {
  return (
    (width, height, offsetX, offsetY, scale) => {
      ...
      const maxValue = minidata.reduce((max, current) => { return Math.max(max, current.count); }, -Infinity); //42
      ...
      details
        ...
        .attr("fill", (d) => {
          ...
          return foundData ? d3.interpolateYlOrRd(foundData.count / maxValue) : d3.interpolateYlOrRd(0);
        })
        .attr("stroke", "gray")
        .attr("d", path)
        .append("title")
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["UserData.json", { url: new URL("../shared/json/UserData.json", import.meta.url), mimeType: "application/json", toString }]
  ]);
